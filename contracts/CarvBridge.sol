...
contract CarvBridge is Ownable {
    ...
    uint256 public constant DECIMAL_CONVERSION_RATE = 1000000000000;
    ...
    function quoteSend(uint32 dstEid, uint256 amount) public view returns (uint256) {
        ...
        uint256 minAmountLD = _removeDust(amountDeductFee);
    ...
    function send(uint32 dstEid, uint256 amount) public payable {
        ...
        (MessagingReceipt memory messagingReceipt,) =
            IOFT(oft).send{value: msg.value}(sendParam, messageFee, msg.sender);

        emit Sent(messagingReceipt.guid, msg.sender, dstEid, amount, amountDeductFee, minAmountLD);
    }
